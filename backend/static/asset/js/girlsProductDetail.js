 ...
 $(document).ready(function(){
    ...
    function loadColor(){
      $.ajax({
        url: base_url +  '/girls-ajax-color-spliter',
        type: "POST",
        dataType:'json',
        data: {
          code:code,
        ...
        success: function(res){
          ...
          if ($(window).width() > 1200) {
            var display_item_no = 3  // set no of item you want to display
          }
          else {
            var display_item_no = 2  // set no of item you want to display
          }
          if(array!=''){
            for(var i=0; i<Math.ceil(array.length/3); i++){
              for(var j=0; j<display_item_no; j++){
